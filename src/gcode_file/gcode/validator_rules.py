...
def require_fields(*field_names: str):
    ...
    def validator(command):
        ...
        if missing:
            raise ValueError(
                f"{command.command} is missing required field(s): {', '.join(missing)}"
            )
...
def require_at_least_one(command: GcodeCommand):
    ...
    if len(command.fields) == 0:
        raise ValueError(
            f"{command.command} requires at least one of the fields, but none were provided."
        )
...
def arc_move_rule(command: GcodeCommand):
    ...
    if (
        not ("I" in command.fields and "J" in command.fields)
        and "R" not in command.fields
    ):
        raise ValueError(
            f"{command.command} requires either I and J fields or an R field."
        )
        ...
        for field_name in field_names:
            if field_name in command.fields:
                if command.fields[field_name] < 0 or command.fields[field_name] > 100:
                    raise ValueError(
                        f"{command.command} {field_name} must be between 0 and 100."
                    )
...
def mutually_exclusive(*field_names: str):
    ...
    def validator(command):
        ...
        if len(present_fields) > 1:
            raise ValueError(
                f"{command.command} fields {', '.join(present_fields)} are mutually exclusive."
            )
    ...
    def validator(command):
        for field in field_names:
            if field in command.fields and command.fields[field] not in [0, 1]:
                raise ValueError(
                    f"{command.command} field {field} must be either 0 or 1."
                )
...
# G4: Dwell
validator.register_rule(
    "G4", {"P": int, "S": int}  # Time in milliseconds  # Time in seconds
)

# G10: Retract
validator.register_rule("G10", {"S": int})  # Tool number

# G11: Unretract
validator.register_rule("G11", {"S": int})  # Tool number
...
validator.register_rule("G32", {"S": int})  # Start calibration

# G33: Delta Auto Calibration
validator.register_rule("G33", {"P": int, "S": int})  # Pattern  # Size

# G34: Set Delta Height
validator.register_rule("G34", {"P": int, "S": int})  # Position  # Size

# G35: Tramming Assistant
validator.register_rule("G35", {"P": bool, "S": bool})  # Pattern  # Size

# G36: Mesh Bed Leveling
validator.register_rule("G36", {"P": bool, "S": bool})  # Pattern  # Size

# G37: Probe Z and Save Mesh
validator.register_rule("G37", {"P": bool, "S": bool})  # Pattern  # Size
...
validator.register_rule("G39", {"P": bool, "S": bool})  # Pattern  # Size
...
# M25: Pause SD print
validator.register_rule("M25", {"U": bool})  # Unload filament when paused
...
# M80: ATX Power On
validator.register_rule("M80", {"S": bool})  # Report the current state and exit
...
# M84: Stop Idle Hold
validator.register_rule("M84", {"S": bool})  # Stop idle hold
...
validator.register_rule(
    "M104",
    {"T": num, "S": num},  # Tool number  # Target temperature
    custom_rule=require_fields("S"),
)

# M106: Turn Fan On
validator.register_rule("M106", {"S": num, "P": num})  # Fan speed (0-255)  # Fan number
...
validator.register_rule(
    "M220", {"S": num}, custom_rule=validate_percentage("S")  # Feedrate Percentage
)

# M221: Set extrusion percentage
validator.register_rule(
    "M221",
    {"T": num, "S": num},  # Tool number  # Extrusion rate Percentage
...
# M552: Set IP Address
validator.register_rule(
    "M552", {"P": str, "S": bool}  # IP address  # Enable/disable network
)
...
validator.register_rule(
    ...
    custom_rule=lambda command: "S" not in command.fields
    or command.fields["S"] in [0, 1],
)
...
validator.register_rule(
    "M701",
    {"T": num, "Z": num, "L": num},  # Tool number  # Z lift height  # Load length
)

# M702: Unload Filament
validator.register_rule(
    "M702",
    {"T": num, "Z": num, "U": num},  # Tool number  # Z lift height  # Unload length
)
...
validator.register_rule(
    ...
    custom_rule=[mutually_exclusive("P", "Q"), validate_binary("A", "F")],
)

# M862.3: Check Model ID
validator.register_rule("M862.3", {"P": num, "T": num})  # Model ID  # Tool number

# M862.4: Check Firmware Version
validator.register_rule(
    "M862.4", {"P": num, "T": num}  # Firmware version  # Tool number
)

# M862.5: Check G-code level
validator.register_rule(
    "M862.5", {"P": int, "Q": num}  # Gcode level  # Current Gcode level
)

# M862.6: Check Firmware Version
validator.register_rule(
    "M862.6", {"P": str, "T": num}  # Firmware version  # Tool number
)

# M900: Linear Advance
validator.register_rule(
    "M900", {"K": num, "T": num}  # Linear advance factor  # Tool number
)
